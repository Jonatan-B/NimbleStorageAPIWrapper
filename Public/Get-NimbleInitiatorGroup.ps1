function Get-NimbleInitiatorGroup {
    param(
        ...
        [Parameter(Position=0, Mandatory, ValueFromPipeline, ValueFromPipelineByPropertyName, ParameterSetName="ListInitiatorGroups")]
        [Switch]
        $List,
        # Should we list all of the InitiatorGroups?
        [Parameter(Position=0, Mandatory, ValueFromPipeline, ValueFromPipelineByPropertyName, ParameterSetName="InitiatorGroupsDetail")]
        [Switch]
        $ListWithDetails,
        # Query for a specific InitiatorGroup ID
        [Parameter(Position=0, Mandatory, ValueFromPipeline, ValueFromPipelineByPropertyName, ParameterSetName="InitiatorGroupId")]
        [String]
        $InitiatorGroupId,
        # Query for a specific InitiatorGroup ID
